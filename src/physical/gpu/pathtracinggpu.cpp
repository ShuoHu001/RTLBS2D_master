...
__global__ void diffractionFindKernel1D(Ray2DGPU* rays, size_t numRays, Wedge2DGPU* wedge, SignedDistanceFieldGPU* sdf, Segment2DGPU* segments, Intersection2DGPU* intersects) {
	...
	if (rayIdx < numRays) {
		if (rays[rayIdx].m_limTotl <= 0 || rays[rayIdx].m_limDiff <= 0)
			return;
		if (rays[rayIdx].IsCaptureByWedgePoint(wedge->m_edge, sdf, segments)) {
...
void PathTraceGPU(const std::vector<Ray2D>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius,  Point2D* dev_rxPositions, size_t numRxPositions, Segment2DGPU* segmentsGPU, size_t numSegments, Wedge2DGPU* wedges, size_t numWedges, SignedDistanceFieldGPU* sdfGPU, std::vector<PathNodeGPU*>& outPathNode) {
...
//3-增加透射后不进行反射、绕射			-完成
//4-增加绕射后不进行透射				-完成
void PathTraceGPUOnlyTree(const std::vector<Ray2D>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, Segment2DGPU* segmentsGPU, size_t numSegments, Wedge2DGPU* wedges, size_t numWedges, SignedDistanceFieldGPU* sdfGPU, std::vector<TreeNodeGPU*>& outTreeNodes)
{
	int threadsPerBlock = 128;
...
bool PathTraceGPULite(RayPathGPU& inpath, const Segment2DGPU* segments)
{
	...
	for (Ray2DGPU& splitray : splitRays) {//遍历所有射线
		...
		while (it != nodes.begin()) {//刨除首尾元素，遍历节点并进行射线追踪
			...
			if (!segments->GetIntersect(iterateRay, &inter)) //若射线与节点面元不相交，放弃追踪路线
				break;
