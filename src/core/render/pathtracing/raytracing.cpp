...
void RayTracing_CPUSingleThread(SYSTEM_MODE systemMode, const std::vector<std::vector<Ray2D>>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, int diffractRayNum, const Scene* scene, std::vector<RayTreeNode*>& vroots)
{
	struct StackItem {
		RayTreeNode* node;
	};
	if (systemMode == MODE_RT) {		//构建射线追踪树结构
		for (size_t i = 0; i < scene->m_transmitters.size(); ++i) {
			...
			while (!stack.empty()) {																		//将栈中所有虚拟根节点进行弹出并进行射线追踪
				...
				stack.pop();
				PathTrace(raySplitFlag, raySplitRadius, diffractRayNum, scene, curItem.node);
			}
			//进行数据综合
...
void RayTracingLBS_CPUSingleThread(SYSTEM_MODE systemMode, const std::vector<std::vector<Ray2D>>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, int diffractRayNum, const Scene* scene, std::vector<RayTreeNode*>& vroots)
{
	struct StackItem {
		RayTreeNode* node;
	};
	for (size_t i = 0; i < scene->m_sensors.size(); ++i) {
		...
		while (!stack.empty()) {																		//将栈中所有虚拟根节点进行弹出并进行射线追踪
			StackItem& curItem = stack.top();
			stack.pop();
			PathTraceLBS(raySplitFlag, raySplitRadius, diffractRayNum, scene, curItem.node);
		}
		//进行数据综合
		RayTreeNode* curVRootNode = vroots[i];
		RayTreeNode* curNode = visualTempNode;
	...
	if (systemMode == MODE_RT) {		//射线追踪模式构建树结构
		for (int i = 0; i < scene->m_transmitters.size(); ++i) {
			...
			for (size_t j = 0; j < rays[i].size(); ++j) {
				...
				auto future = pool.enqueue(PathTrace, raySplitFlag, raySplitRadius, diffractRayNum, scene, vrootTreeNode);
...
void RayTracingLBS_CPUMultiThread(SYSTEM_MODE systemMode, const std::vector<std::vector<Ray2D>>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, int diffractRayNum, const Scene* scene, uint16_t threadNum, std::vector<RayTreeNode*>& vroots)
{
	...
	for (int i = 0; i < scene->m_sensors.size(); ++i) {
		...
		for (size_t j = 0; j < rays[i].size(); ++j) {
			...
			vrootPathNode->m_nextRay = rays[i][j];
			RayTreeNode* vrootTreeNode = new RayTreeNode(vrootPathNode);
			auto future = pool.enqueue(PathTraceLBS, raySplitFlag, raySplitRadius, diffractRayNum, scene, vrootTreeNode);
			vrootTemp[j] = vrootTreeNode;
		}
...
void RayTracing_GPUMultiThread(const std::vector<std::vector<Ray2D>>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, int diffractRayNum, const Scene* scene, std::vector<std::vector<PathNodeGPU*>>& gpuPathNodes)
{
	...
	for (size_t i = 0; i < txNum; ++i) {
		PathTraceGPU(rays[i], limitInfo, raySplitFlag, raySplitRadius, diffractRayNum, thrust::raw_pointer_cast(device_RxPositions.data()), rxNum, thrust::raw_pointer_cast(device_segments.data()), segmentNum,
...
void RayTracing_GPUMultiThreadWithNode(const std::vector<std::vector<Ray2D>>& rays, const LimitInfo& limitInfo, bool raySplitFlag, RtLbsType raySplitRadius, int diffractRayNum, const Scene* scene, std::vector<std::vector<TreeNodeGPU*>>& gpuTreeNodes)
{
	...
	for (size_t i = 0; i < sensorNum; ++i) {
		PathTraceGPUOnlyTree(rays[i], limitInfo, raySplitFlag, raySplitRadius, diffractRayNum, thrust::raw_pointer_cast(device_segments.data()), segmentNum,
