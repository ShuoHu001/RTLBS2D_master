#include "raypathsettergpu.h"

__constant__ int D_CONST_RAYPATHSETTER_MAXNODESIZE;												/** @brief	L1设备快速内存-最大深度	*/
__constant__ RtLbsType D_CONST_RAYPATHSETTER_RAYSPLITRADIUS;										/** @brief	L1设备快速内存-射线分裂半径	*/
...
__global__ void CheckTargetInsideKernel_RayPathSetter(CPUConverterPathNode* allNodes, int numNodes, Point2D* target, int* nodeIds)
{
	...
	if (nodeIdx < numNodes) {
		...
		if (curNode.IsCapturedByPoint(*target, D_CONST_RAYPATHSETTER_RAYSPLITRADIUS, fatherNode)) {			//若被接收，则加入nodesId中
...
__global__ void FindPathNodeKernel_RayPathSetter(CPUConverterPathNode* allNodes, int numNodes, int* nodeIds, int numId, int* pathNodeIds, int numPathId)
{
	...
	if (idx < numId) {
		...
		int pathNodeId = (idx + 1) * D_CONST_RAYPATHSETTER_MAXNODESIZE - 1;										/** @brief	设计从末尾开始进行写入，进行追根溯源	*/
		pathNodeIds[pathNodeId] = curNodeId;
		int fatherNodeId = allNodes[curNodeId].m_fatherNodeId;
		for (int i = 0; i < D_CONST_RAYPATHSETTER_MAXNODESIZE - 1; ++i) {
			if (fatherNodeId == -1) {
				break;
			}
...
void DirectlySetResultPath_GPUMultiThread(const std::vector<RayTreeNode*>& vroots, const Scene* scene, RtLbsType splitRaidus, std::vector<GSPairCluster>& clusters)
{
	...
	//拷贝常量至GPU快速内存中
	cudaMemcpyToSymbol(D_CONST_RAYPATHSETTER_MAXNODESIZE, &maxNodeSize, sizeof(int));
	cudaMemcpyToSymbol(D_CONST_RAYPATHSETTER_RAYSPLITRADIUS, &splitRaidus, sizeof(RtLbsType));
	...
	for (int i = 0; i < static_cast<int>(dev_TargetPositions.size()); ++i) {
		...
		//进行目标捕获判定
		CheckTargetInsideKernel_RayPathSetter CUDA_KERNEL(numBlock1, threadPerBlock)(thrust::raw_pointer_cast(dev_AllPathNodes.data()), num_Nodes, thrust::raw_pointer_cast(&dev_TargetPositions[i]), thrust::raw_pointer_cast(dev_Temp_TargetNodeIds.data()));
		cudaerr = cudaDeviceSynchronize();
		if (cudaerr != cudaSuccess) {
			LOG_ERROR << "RayPathSetterGPU: CheckTargetInsideKernel " << cudaGetErrorString(cudaerr) << ENDL;
		}
	...
	FindPathNodeKernel_RayPathSetter CUDA_KERNEL(numBlock2, threadPerBlock)(thrust::raw_pointer_cast(dev_AllPathNodes.data()), num_Nodes, thrust::raw_pointer_cast(dev_TargetNodeIds.data()), dev_TargetNodeIds.size(), thrust::raw_pointer_cast(dev_TargetPathNodeIds.data()), dev_TargetPathNodeIds.size());
	cudaerr = cudaDeviceSynchronize();
	if (cudaerr != cudaSuccess) {
		LOG_ERROR << "RayPathSetterGPU: FindPathNodeKernel " << cudaGetErrorString(cudaerr) << ENDL;
	}
	...
	for (auto curTargetNodeNum : everyTargetNodesNum) {													//遍历每个目标
		...
		for (int i = 0; i < curTargetNodeNum; ++i) {
			...
			//验证路径的有效性
			if (RectifyRayPath(scene, newRayPath, targetPoints[pId])) {
