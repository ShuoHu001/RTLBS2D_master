...
PathInfo::PathInfo(const PathInfo& pathInfo)
	...
	, m_powerLin(pathInfo.m_powerLin)
...
void PathInfo::CalculateBaseInfo(RtLbsType freq, const std::vector<Complex>& tranFunctionData, Transmitter* transmitter, Receiver* receiver)
{
	//赋值
	m_freq = freq;
	...
	if (m_rayPathType != RAYPATH_TERRAIN_DIFFRACTION) {			//常规路径电磁计算方法
		m_vectorEField = CalculateStrengthField(m_rayPath, transmitter->m_power, freq, tranFunctionData, transmitter->m_antenna, receiver->m_antenna);
		...
		m_vectorEField = CalculateTerrainDiffractionEField(m_terrainDiffractionPath, transmitter->m_power, freq, tranFunctionData, transmitter->m_antenna, receiver->m_antenna);
		m_scalarEField = m_vectorEField.MValue();
	...
	m_powerLin = std::pow(10.0, m_power / 10.0);													//计算线性功率
	m_magnitude.Init(pow(10, m_power / 10), m_phaseOffset);											//计算复数强度
}

void PathInfo::CalculateBaseInfo(RtLbsType power, RtLbsType freq, const std::vector<Complex>& tranFunctionData, const AntennaLibrary* antLibrary, const Sensor* sensor)
{
	//赋值
	m_freq = freq;
	RtLbsType lamda = LIGHT_VELOCITY_AIR / freq;				//波长，单位m
	...
	//1-计算电磁场
	if (m_rayPathType != RAYPATH_TERRAIN_DIFFRACTION) {			//常规路径电磁计算方法
		m_vectorEField = CalculateStrengthField(m_rayPath, power, freq, tranFunctionData, omniAntenna, sensor->m_antenna);
		m_scalarEField = m_vectorEField.MValue();
		m_timeDelay = m_rayPath->GetPropagationTime();
		m_phaseOffset = m_rayPath->GetPhaseOffset(freq);
		m_aoDPhi = m_rayPath->GetAOD_Phi();
		...
		m_vectorEField = CalculateTerrainDiffractionEField(m_terrainDiffractionPath, power, freq, tranFunctionData, omniAntenna, sensor->m_antenna);
		m_scalarEField = m_vectorEField.MValue();
		m_timeDelay = m_terrainDiffractionPath->GetPropagationTime();
		m_phaseOffset = m_terrainDiffractionPath->GetPhaseOffset(freq);
		m_aoDPhi = m_terrainDiffractionPath->GetAngleofDeparture_Phi();
